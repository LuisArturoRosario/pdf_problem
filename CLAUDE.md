...
- Input is ALWAYS a PDF — no other input types. Do not build ingestion abstractions for other formats. NOTE: PDFs include both native-text (tabular) and scanned PDFs; scanned files will require OCR and are in project scope even though OCR is deferred past the demo sprint (see Immutable Rules #6).
...
6. Scope discipline: OCR for scanned PDFs, multi-format coverage, and pipeline packaging are OUT of scope for the demo sprint (but in scope for the overall project) unless explicitly re-scoped.
7. Accuracy priority: location/address data is the highest-value field and should be as close to 100% correct as possible; ~95% confidence is acceptable for other fields. Never sacrifice location accuracy.
8. DO NOT use or share the Redlands PD sample — its redactions were reversible and the stakeholder asked that its data not be circulated. Exclude it from committed samples and outputs.
...
- Performance is not a primary concern: workday turnaround is fine (minutes per file is acceptable, even for 700–800 page PDFs). Prioritize correctness over speed.
